...
while watchedSecs < 28 * 60:
    ...
    for pos in lst:
        doneIcon = pyautogui.screenshot(region=(int(pos.left + 275), int(pos.top - 18), 28, 28))
        if not util.blankImage(doneIcon):
            continue
        
        selected = True
    
        vidLenImg = pyautogui.screenshot(region=(int(pos.left + 20), int(pos.top), 75, 20))
        vidSecs, vidLen = util.ocrVidSecs(vidLenImg)
        print(f"selected video, len={vidLen}, secs={vidSecs}")
    
        pyautogui.click(pos.left + 20, pos.top, duration=util.moveDuration(), tween=pyautogui.easeInOutQuad)
        time.sleep(3)
        util.startPlayVideo()
        ...
        while datetime.datetime.now() < finishTime:
            if pyautogui.locateOnScreen("assets/singleQuiz.png") != None:
                ...
                for btn in btns:
                    pyautogui.click(btn.left + 10, btn.top + 5, duration=util.moveDuration(), tween=pyautogui.easeInOutQuad)
                    time.sleep(0.2)

                    try:
                        if pyautogui.locateOnScreen("assets/singleQuizPass.png") != None:
